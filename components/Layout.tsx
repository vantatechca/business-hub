...
import {
  LayoutDashboard, TrendingUp, Layers, Users, CheckSquare,
  Calendar, DollarSign, CreditCard, Target,
  LogOut, Zap, Bell, Search, Plus, Check,
} from "lucide-react";
import { Avatar } from "./ui/shared";

const NAV = [
  { s:"MAIN",       items:[{id:"dashboard",l:"Dashboard",h:"/dashboard",I:LayoutDashboard},{id:"analytics",l:"Analytics",h:"/analytics",I:TrendingUp}] },
  { s:"OPERATIONS", items:[{id:"departments",l:"Departments",h:"/departments",I:Layers},{id:"team",l:"Team",h:"/team",I:Users},{id:"tasks",l:"Tasks",h:"/tasks",I:CheckSquare},{id:"checkin",l:"Check-Ins",h:"/checkin",I:Calendar}] },
  { s:"FINANCE",    items:[{id:"revenue",l:"Revenue",h:"/revenue",I:DollarSign},{id:"expenses",l:"Expenses",h:"/expenses",I:CreditCard},{id:"goals",l:"Goals",h:"/goals",I:Target}] },
];
...
export default function AppLayout({ children, title, onNew, newLabel="New" }: { children:React.ReactNode; title:string; onNew?:()=>void; newLabel?:string; }) {
  ...
  // KEY FIX: mounted guard prevents hydration mismatch on all theme/icon conditionals
      ...
      <div style={{display:"flex",height:"100vh",overflow:"hidden",background:"var(--bg-base)"}}>
        ...
        <aside style={{width:col?"var(--sidebar-collapsed)":"var(--sidebar-width)",background:"var(--bg-sidebar)",borderRight:"1px solid var(--border-sidebar)",display:"flex",flexDirection:"column",transition:"width .2s ease",flexShrink:0,overflow:"hidden"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,padding:"13px 12px",borderBottom:"1px solid var(--border-sidebar)",flexShrink:0,minHeight:56}}>
            <div style={{width:32,height:32,borderRadius:9,flexShrink:0,background:"linear-gradient(135deg,#5b8ef8,#6366f1)",display:"flex",alignItems:"center",justifyContent:"center"}}>
              <Zap size={15} color="#fff"/>
            </div>
            {!col&&<div><div style={{fontSize:13,fontWeight:800,color:"var(--text-primary)",whiteSpace:"nowrap",letterSpacing:"-0.02em"}}>Business Hub</div><div style={{fontSize:10,color:"var(--text-secondary)"}}>Command Center</div></div>}
          </div>
          ...
          <div style={{padding:8,borderTop:"1px solid var(--border-sidebar)",flexShrink:0}}>
            {/* FIXED: use text emoji not Lucide SVG — avoids <circle> hydration mismatch */}
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,padding:col?"6px 0":"6px 10px"}}>
              <Avatar s={ini} size={28}/>
              {!col&&<div><div style={{fontSize:12,fontWeight:700,color:"var(--text-primary)"}}>{name}</div><div style={{fontSize:10,color:"var(--text-secondary)"}}>Admin</div></div>}
            </div>
            <button onClick={()=>signOut({callbackUrl:"/login"})} className={`nav-item ${col?"justify-center":""}`} style={{fontSize:12,marginTop:2}}>
              <LogOut size={14}/>{!col&&" Sign out"}
            </button>
            {/* FIXED: text arrows instead of ChevronLeft/Right SVG icons */}
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
          ...
          <main style={{flex:1,overflowY:"auto",padding:"16px 18px"}} className="animate-fade-in">
            {children}
          </main>
        </div>
      </div>
